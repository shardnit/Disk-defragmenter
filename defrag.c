...
unsigned char  *
read_bitmap(char *fs, int fd, int blocks, int offset)
{
...
void first_indirect_block_copy(int src_fs_fd, zone_t src_blk, int dst_fs_fd, zone_t dst_blk, struct super_block *src_sb, struct super_block *dst_sb)
{
	...
	for(i=0; i< V2_INDIRECTS(BLOCK_SIZE); i++)
	{
		if(src_indirect_buf[i] != NO_ZONE)
		{
			...
			dst_indirect_buf[i] = (zone_t)NO_ZONE;
	}
...
main(int argc, char **argv)
{
	...
	dst_inode_bitmap = read_bitmap(dst_file_system, dst_fs_fd, BLOCK_SIZE * dst_imap_blocks, BLOCK_SIZE*(START_BLOCK+0));
	dst_zone_bitmap = read_bitmap(dst_file_system, dst_fs_fd, BLOCK_SIZE * dst_zmap_blocks, BLOCK_SIZE*(START_BLOCK+dst_imap_blocks));
	...
	src_inode_bitmap = read_bitmap(src_file_system, src_fs_fd, BLOCK_SIZE * src_imap_blocks, BLOCK_SIZE*(START_BLOCK+0));
	if (src_last_inode(src_inode_bitmap, BLOCK_SIZE * src_imap_blocks) > (8 * BLOCK_SIZE * (dst_super_block->s_imap_blocks))) {
		print_error("Source file system larger than Destination FS(inodes):", dst_file_system, ERR_SMALLER_TARGET_FS);
	}
	/*
	...
	src_zone_bitmap = read_bitmap(src_file_system, src_fs_fd, BLOCK_SIZE * src_zmap_blocks, BLOCK_SIZE*(START_BLOCK+src_imap_blocks));
	if (num_set_bits(src_zone_bitmap, BLOCK_SIZE * src_zmap_blocks) > (((dst_super_block->s_zones - dst_super_block->s_firstdatazone) << dst_super_block->s_log_zone_size) + 1)) {
		print_error("Source file system larger than Destination FS(zones):", dst_file_system, ERR_SMALLER_TARGET_FS);
	}
	...
	for(i=0; i< src_imap_blocks*BLOCK_SIZE ; i++)
	{
		for(k=1; k<=8; k++)
		{
			/* skip the first inode */
			if(i==0 && k==1)
				continue;

			/* if this inode exists i.e. if this bit is *SET* */
			if(check_bit(src_inode_bitmap[i], k))
			{
				inode_num = (i*8)+k-1;

				/* now read this inode */
				if(lseek(src_fs_fd, src_inode_fd+((inode_num-1)*INODE_SIZE), SEEK_SET)==-1)
				...
				if(((src_inode->d2_mode & I_TYPE) == I_REGULAR) || ((src_inode->d2_mode & I_TYPE) == I_DIRECTORY))
				{
					...
					for(j=0; j< V2_NR_DZONES; j++)
					{
						if(src_inode->d2_zone[j]!= NO_ZONE)
						{
							...
							dst_inode->d2_zone[j]=(zone_t)NO_ZONE;
					}
					...
					if(src_inode->d2_zone[V2_NR_DZONES] != NO_ZONE)
					{
						...
						dst_inode->d2_zone[V2_NR_DZONES]= (zone_t)NO_ZONE;
					...
					if(src_inode->d2_zone[V2_NR_DZONES+1] != NO_ZONE)
					{
						...
						dst_inode->d2_zone[V2_NR_DZONES+1] = (zone_t)NO_ZONE;
				...
				lseek(dst_fs_fd, dst_inode_fd + ((inode_num-1)*INODE_SIZE), SEEK_SET);
				...
				dst_inode_bitmap[(inode_num/8)] = set_bit(dst_inode_bitmap[(inode_num/8)], (inode_num - (8 * (inode_num/8))+1)); 
			}
